...
class DSREWorker(QtCore.QThread):
    ...
    def run(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(os.cpu_count()*9999) as executor:
            for idx, in_path in enumerate(self.files, start=1):
                if self._abort:
                    break
                work_list.append(executor.submit(work, idx, in_path, total))
            for th in work_list:
                if self._abort:
                    try:
                        th.shutdown(wait=False, cancel_futures=True)
                    except:
                        pass
                th.result()
            self.sig_overall_progress.emit(done, total)
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(self.tr("DSRE v3.1.1-Termux"))
