...
def add_ffmpeg_to_path():
    if hasattr(sys, "_MEIPASS"):  # Temporary directory after packaging
        ...
        ffmpeg_dir = "/Applications/DSRE.app/Contents/Frameworks/ffmpeg"
...
def save_wav24_out(in_path, y_out, sr, out_path, fmt="ALAC", normalize=True):
    ...
    try:
        ...
        if fmt == "MP3":
            ...
            cmd = 'ffmpeg -i "{}" -i "{}" -map 0:a -map 1:v? -map_metadata 1 -c:a {} -y -sample_fmt {} -b:a 320K -c:v copy "{}"'.format(tmp_wav.name, in_path, codec_map[fmt], sample_fmt_map[fmt], out_path)
        else:
            # Extract cover image
            try:
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(self.tr("DSRE v2.0.Enhanced - Audio Enhancement Suite"))
        ...
        self.status_bar.showMessage(self.tr("Ready - Enhanced Audio Processing Algorithm Loaded"))
        ...
        self.sb_sr.setToolTip(self.tr("Target sample rate: Higher rates preserve more frequency content"))
        ...
        self.lbl_stats = QtWidgets.QLabel(self.tr("Ready to process - Enhanced Algorithm Active"))
        ...
        self.append_log(self.tr("• Multi-band harmonic excitement for richer sound"))
        self.append_log(self.tr("• Psychoacoustic enhancement targeting human hearing"))
        self.append_log(self.tr("• Dynamic range enhancement for more liveliness"))
        self.append_log(self.tr("• Stereo width enhancement for immersive soundstage"))
        self.append_log(self.tr("• Intelligent frequency-dependent processing"))
        self.append_log("=" * 50)
        ...
        if self.dark_mode:
            self.btn_test_dark.setText(self.tr("Light Mode"))
        else:
            self.btn_test_dark.setText(self.tr("Dark Mode"))
    ...
    def show_about(self):
        """Show about dialog"""
        QtWidgets.QMessageBox.about(self, self.tr("""About DSRE Enhanced 
...
Discussion Group: 323861356 (QQ)"""), self.tr("""About DSRE Enhanced 
DSRE v2.0 - Enhanced Audio Processing Suite\n\n
Enhanced with advanced multi-band harmonic excitement,\n
psychoacoustic enhancement, and dynamic range processing.\n\n
Features:\n
...
Discussion Group: 323861356 (QQ)"""))
    ...
    def on_clear_files(self):
        ...
        self.append_log(self.tr("DSRE v2.0 - Enhanced Audio Processing Suite"))
        self.append_log(self.tr("Ready for enhanced audio processing!"))
    ...
    def on_processing_stats(self, stats):
        ...
        if stats['start_time']:
            ...
            if processed > 0:
                ...
                self.lbl_eta.setText(self.tr(f"Elapsed: {elapsed_str} | ETA: {eta_str}"))
            else:
...
def main():
    ...
    qm_file = '/Applications/DSRE.app/Contents/Resources/lang/ja_JP.qm'
